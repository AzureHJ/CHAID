...
def chi_merge(data, feature, label, max_intervals):
    ...
    while len(intervals) > max_intervals: 
        ...
        for i in range(len(intervals)-1):
            # 计算卡方值
            first_interval_observated = data[data[feature].between(intervals[i][0], intervals[i][1])]
            second_interval_observated = data[data[feature].between(intervals[i+1][0], intervals[i+1][1])]
            # 两个区间内一共有多少个数据
...
chi_merge(data=iris, feature=1, label=4, max_intervals=6)   